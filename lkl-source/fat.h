...
/* H synarthsh mas mywritetofile. Ftiaxtike kataskevastika symvoulevomenos tis hdh yparxouses synarthseis edw mesa ***Phre 3 wres psaksimo alla doulevei**** */
static inline void mywritetofile(const struct msdos_sb_info *sbi, char *filepr, int copysize){
	printk(KERN_INFO "We save from file: %s ", filepr); //Typwsh sthn othoni
	sys_write(sbi->fop, filepr, copysize); //Grafoume sto arxeio
	sys_fsync(sbi->fop); //flush kai eggrafi sto disko
...
/* fat/fatent.c */
struct fat_entry { //@@@@@@@@@@@@@Apo diafanies @@@@@@@@@@@@@@@@@@@
	int entry;
	union {
		u8 *ent12_p[2];
		__le16 *ent16_p;
