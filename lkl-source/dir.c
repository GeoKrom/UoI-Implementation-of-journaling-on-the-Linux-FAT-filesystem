...
found:
	...
	sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: slot_off = %lld, nr_slots = %d, de = %hhn, bh = %lu, i_pos = %lld \n",sinfo->slot_off,sinfo->nr_slots,sinfo->de->name,sinfo->bh->b_state,sinfo->i_pos);
			     ...
			     int short_only, int both)
{
	...
	if (!IS_DEADDIR(inode)) {
		...
		//Kratame tis allages gia eggrafh sto arxeio
		sprintf(fileprinter, "fs/fat/dir.c(file) => Changed: f_pos = %lld\n",file->f_pos);
		mywritetofile(sbi, fileprinter, CPSIZE);
...
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb); //to orizoume gia th xrhsh tou fop.
	...
	//Kratame tis allages gia eggrafh sto arxeio
	sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: slot_off = %lld, bh = %p \n",sinfo->slot_off, NULL);
	mywritetofile(sbi, fileprinter, CPSIZE);
	...
	while (fat_get_short_entry(dir, &sinfo->slot_off, &sinfo->bh,
		...
		if (!strncmp(sinfo->de->name, name, MSDOS_NAME)) {
			...
			//Kratame tis allages gia eggrafh sto arxeio
			sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: slot_off = %lld, nr_slots = %d, i_pos = %lld \n",sinfo->slot_off,sinfo->nr_slots,sinfo->i_pos);
			mywritetofile(sbi, fileprinter, CPSIZE);
	...
	kfree(fileprinter); //Apodemseysi xwrou pinaka.
	return -ENOENT;
}
...
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb); //TO orizoume gia th xrhsh fop.
	char *fileprinter; //Voithitikos pinakas.
	fileprinter = kzalloc(CPSIZE, GFP_NOWAIT); //Desmeysi xwrou.
...
int fat_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
{
	...
	//Kratame tis allages gia eggrafh sto arxeio
	sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: nr_slots = %d, de = %hhn, bh = %p \n",sinfo->nr_slots, sinfo->de->name, NULL);
	mywritetofile(sbi, fileprinter, CPSIZE);
		    ...
		    struct fat_slot_info *sinfo)
{
	...
	//Kratame tis allages gia eggrafh sto arxeio
	sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: nr_slots = %d \n",sinfo->nr_slots);
	mywritetofile(sbi, fileprinter, CPSIZE);
...
found:
	...
	//Kratame tis allages gia eggrafh sto arxeio
	sprintf(fileprinter, "fs/fat/dir.c(fat_slot_info) => Changed: slot_off = %lld, de = %s bh = %lu, i_pos = %lld \n",sinfo->slot_off, sinfo->de->name, sinfo->bh->b_state, sinfo->i_pos);
	mywritetofile(sbi, fileprinter, CPSIZE);
