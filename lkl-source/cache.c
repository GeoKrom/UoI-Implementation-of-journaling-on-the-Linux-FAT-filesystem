...
static void __fat_cache_inval_inode(struct inode *inode)
{
	...
	struct super_block *sb = inode->i_sb; //prosthetoume edw mia arxikopoihsh tou sb gia na kanoume ta writes
	struct msdos_sb_info *sbi = MSDOS_SB(sb); // mazi me me to struct msdos_sb_info
	...
	while (!list_empty(&i->cache_lru)) {
		...
		sprintf(fileprinter, "fs/fat/cache.c(msdos_inode_info) => Changed: nr_caches = %d \n",i->nr_caches);
	...
	i->cache_valid_id++;
	//Kratame tis allages gia eggrafh sto arxeio
	sprintf(fileprinter, "fs/fat/cache.c(msdos_inode_info) => Changed: cache_valid_id = %d \n",i->cache_valid_id);
	mywritetofile(sbi, fileprinter, CPSIZE);
